...
RUN mkdir -p /usr/src/sider
COPY transformers/sider/python-flask-server /usr/src/sider
WORKDIR /usr/src/sider
RUN python setup.py bdist_wheel
...
COPY transformers/sider/python-flask-server/info /usr/src/app/info
...
ADD https://translator.broadinstitute.org/db/sider.sqlite /usr/src/app/data
WORKDIR /usr/src/app
COPY --from=packaging-image /usr/src/base/dist .
COPY --from=packaging-image /usr/src/sider/dist .
RUN pip3 install -I base_transformer-2.0.1-py3-none-any.whl
RUN pip3 install -I sider_transformer-2.4.0-py3-none-any.whl
COPY transformers/sider/python-flask-server/requirements.txt .
