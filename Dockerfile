...
RUN mkdir -p /usr/src/drugcentral
COPY transformers/drugcentral/python-flask-server /usr/src/drugcentral
WORKDIR /usr/src/drugcentral
RUN python setup.py bdist_wheel
...
COPY transformers/drugcentral/python-flask-server/info /usr/src/app/info
...
ADD https://translator.broadinstitute.org/db/DrugCentral.sqlite /usr/src/app/data
WORKDIR /usr/src/app
COPY --from=packaging-image /usr/src/base/dist .
COPY --from=packaging-image /usr/src/drugcentral/dist .
RUN pip3 install -I drug_central_transformer-2.3.2-py3-none-any.whl
RUN pip3 install -I base_transformer-2.0.0-py3-none-any.whl
COPY transformers/drugcentral/python-flask-server/requirements.txt .
