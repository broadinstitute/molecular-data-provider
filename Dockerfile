...
RUN mkdir -p /usr/src/ctd
COPY transformers/ctd/python-flask-server /usr/src/ctd
WORKDIR /usr/src/ctd
RUN python setup.py bdist_wheel
...
COPY transformers/ctd/python-flask-server/info /usr/src/app/info
COPY transformers/ctd/python-flask-server/conf /usr/src/app/conf
...
ADD https://translator.broadinstitute.org/db/CTD.sqlite /usr/src/app/data
WORKDIR /usr/src/app
COPY --from=packaging-image /usr/src/base/dist .
COPY --from=packaging-image /usr/src/ctd/dist .
RUN pip3 install -I ctd_transformer-2.5.0-py3-none-any.whl
RUN pip3 install -I base_transformer-2.5.0-py3-none-any.whl
COPY transformers/ctd/python-flask-server/requirements.txt .
RUN pip3 install --no-cache-dir -r requirements.txt
EXPOSE 8080
CMD ["nohup","gunicorn", "-w","24","-b","0.0.0.0:8080","openapi_server.__main__:app","--timeout","300"]
