...
RUN mkdir -p /usr/src/probe-miner
COPY transformers/probe-miner/python-flask-server /usr/src/probe-miner
WORKDIR /usr/src/probe-miner
RUN python setup.py bdist_wheel
...
COPY transformers/probe-miner/python-flask-server/info /usr/src/app/info
...
ADD https://translator.broadinstitute.org/db/probeminer.sqlite /usr/src/app/data
WORKDIR /usr/src/app
COPY --from=packaging-image /usr/src/base/dist .
COPY --from=packaging-image /usr/src/probe-miner/dist .
RUN pip3 install -I base_transformer-2.0.1-py3-none-any.whl
RUN pip3 install -I probe_miner_transformer-2.4.0-py3-none-any.whl
COPY transformers/probe-miner/python-flask-server/requirements.txt .
