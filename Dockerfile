...
COPY --from=packaging-image /usr/src/base/target/universal/molecular-data-provider-2.4.1.tgz /usr/src/target
RUN gunzip molecular-data-provider-2.4.1.tgz
RUN tar xf molecular-data-provider-2.4.1.tar
WORKDIR /usr/src/target/molecular-data-provider-2.4.1
EXPOSE 9200
