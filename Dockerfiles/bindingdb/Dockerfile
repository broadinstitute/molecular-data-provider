...
RUN mkdir -p /usr/src/bindingdb
COPY transformers/bindingdb/python-flask-server /usr/src/bindingdb
WORKDIR /usr/src/bindingdb
RUN python setup.py bdist_wheel
...
COPY transformers/bindingdb/python-flask-server/info /usr/src/app/info
...
ADD https://translator.broadinstitute.org/db/BindingDB.sqlite /usr/src/app/data
WORKDIR /usr/src/app
