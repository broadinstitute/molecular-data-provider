...
RUN mkdir -p /usr/src/bigg-models
COPY transformers/bigg-models/python-flask-server /usr/src/bigg-models
WORKDIR /usr/src/bigg-models
RUN python setup.py bdist_wheel
...
COPY transformers/bigg-models/python-flask-server/info /usr/src/app/info
...
ADD https://translator.broadinstitute.org/db/BiGG.sqlite /usr/src/app/data
WORKDIR /usr/src/app
COPY --from=packaging-image /usr/src/base/dist .
COPY --from=packaging-image /usr/src/bigg-models/dist .
RUN pip3 install -I bigg_models_transformer-2.3.0-py3-none-any.whl
RUN pip3 install -I base_transformer-2.0.0-py3-none-any.whl
COPY transformers/bigg-models/python-flask-server/requirements.txt .
