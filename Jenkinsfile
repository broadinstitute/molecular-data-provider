pipeline {
    ...
    stages {
        ...
        stage('build') {
            ...
            steps {
                withEnv([
                    "IMAGE_NAME=translator-molepro-dgidb",
        ...
        stage('Deploy') {
            ...
            steps {
                sshagent (credentials: ['labshare-svc']) {
                    dir(".") {
                        sh 'git clone -b dgidb-ci-update git@github.com:Sphinx-Automation/translator-ops.git'
                        withAWS(credentials:'aws-ifx-deploy') {
                            sh '''
                            aws --region ${AWS_REGION} eks update-kubeconfig --name ${KUBERNETES_CLUSTER_NAME}
                            cp -R translator-ops/ops/molepro/deploy/* translator-ops/ops/molepro/helm/                           
                            cp -R translator-ops/ops/molepro/config/transformers/molepro-dgidb.yaml translator-ops/ops/molepro/helm/
