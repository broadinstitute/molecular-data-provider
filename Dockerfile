...
RUN mkdir -p /usr/src/kinomescan
COPY transformers/kinomescan/python-flask-server /usr/src/kinomescan
WORKDIR /usr/src/kinomescan
RUN python setup.py bdist_wheel
...
COPY transformers/kinomescan/python-flask-server/info /usr/src/app/info
...
ADD https://translator.broadinstitute.org/db/KinomeScan.sqlite /usr/src/app/data
WORKDIR /usr/src/app
COPY --from=packaging-image /usr/src/base/dist .
COPY --from=packaging-image /usr/src/kinomescan/dist .
RUN pip3 install -I base_transformer-2.5.1-py3-none-any.whl
RUN pip3 install -I kinomescan_transformer-2.5.0-py3-none-any.whl
COPY transformers/kinomescan/python-flask-server/requirements.txt .
