...
public class HTTP {
	...
	public static String post(final URL url) throws IOException {
		final HttpURLConnection con = (HttpURLConnection)url.openConnection();
	...
	private static String response(final URL url, final HttpURLConnection con) throws IOException {
		...
		try {
			final ErrorMsg errorMsg = mapper.readValue(readResponse(con.getErrorStream()), ErrorMsg.class);
			msg = msg + " " + errorMsg.title + ": " + errorMsg.detail;
		}
		catch (Exception e) {
