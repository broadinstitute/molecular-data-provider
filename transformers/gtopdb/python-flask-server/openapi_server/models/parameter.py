...
class Parameter(Model):
    ...
    def allowed_range(self, allowed_range):
        ...
        :type allowed_range: List[float]
        """
        if allowed_range is not None and len(allowed_range) > 2:
            raise ValueError("Invalid value for `allowed_range`, number of items must be less than or equal to `2`")  # noqa: E501
        if allowed_range is not None and len(allowed_range) < 2:
