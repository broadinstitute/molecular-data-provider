...
RUN mkdir -p /usr/src/pharmgkb
COPY transformers/pharmgkb/python-flask-server /usr/src/pharmgkb
WORKDIR /usr/src/pharmgkb
RUN python setup.py bdist_wheel
...
COPY transformers/pharmgkb/python-flask-server/info /usr/src/app/info
...
ADD https://translator.broadinstitute.org/db/pharmgkb.sqlite /usr/src/app/data
WORKDIR /usr/src/app
COPY --from=packaging-image /usr/src/base/dist .
COPY --from=packaging-image /usr/src/pharmgkb/dist .
RUN pip3 install -I base_transformer-2.5.1-py3-none-any.whl
RUN pip3 install -I pharmgkb_transformer-2.5.0-py3-none-any.whl
COPY transformers/pharmgkb/python-flask-server/requirements.txt .
